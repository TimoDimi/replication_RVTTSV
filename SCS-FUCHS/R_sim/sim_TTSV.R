...
sim_TTSV_GeneralDependence <- function(days, lambda_det, varsigma_det, TT=23401, Sigma=diag(3)){
  ...
  for (i_day in 1:days){
    ...
    ### Simulation increments of lambda, varismg and B jointly here!
    # ToDo: How do I do that? I need dependence at time t!!!! which is complicated here...
    # dW_vec <- MASS::mvrnorm(TT, mu=rep(0,3), Sigma=Sigma)
    # dW_lambda <- dW_vec[,1]
    dW_lambda <- rnorm(TT)
    ...
    Lambda_T <- tail(intensities_day$Lambda,1) - head(intensities_day$Lambda,1)
    n <- rpois(1,Lambda_T)
    
    Ft <- (cumsum(intensities_day$lambda) - head(intensities_day$Lambda,1))/Lambda_T
    ...
    prices_day <- tibble(Date=i_day,
                         SecSinceStart=unique(sort(Ft_inv(runif(n)))))
    ...
    Sigma_2by2 <- Sigma[2:3,2:3]
    dW_vec <- MASS::mvrnorm(n_ticks, mu=rep(0,2), Sigma=Sigma_2by2)
    dW_varsigma <- dW_vec[tick_times_day_floor, 2] + 
    
    
    tick_times_day_exact <- prices_day$SecSinceStart
    tick_times_day_floor <- pmax(1,floor(tick_times_day_exact))
    
    dW_varsigma <- (tick_times_day_exact-tick_times_day_floor) * dW_vec[tick_times_day_floor, 2] + 
      (1-(tick_times_day_exact-tick_times_day_floor)) * dW_vec[tick_times_day_floor+1, 2]
    
    dW_price <- (tick_times_day_exact-tick_times_day_floor) * dW_vec[tick_times_day_floor, 3] + 
      (1-(tick_times_day_exact-tick_times_day_floor)) * dW_vec[tick_times_day_floor+1, 3]
    
    # tick_times_day <- pmax(1,round(prices_day$SecSinceStart))
    ...
    # Problem: High dependence due to rounding!!!
    # dW_varsigma <- dW_vec[tick_times_day,2]
    # dW_price <- dW_vec[tick_times_day,3]
