...
# end_time <- Sys.time()
# (run_time_intensity <- mid_time - start_time)
# (run_time_HTS <- end_time - mid_time)


# Empirically, M * delta^1.5 stays approximately constant for HTS!!!
# 
# df_resample_HTS_guess <- resample_prices_HTS(dat_Sample,
#                                              delta_set= 10^c(-2,-3),
#                                              max_returns=max_returns_HTS) 
# 
# df_resample_HTS_guess %>% 
#   group_by(delta) %>% 
#   summarize(M_avg = mean(M_individual),
#             constant_approx = M_avg * mean(delta)^1.8)
