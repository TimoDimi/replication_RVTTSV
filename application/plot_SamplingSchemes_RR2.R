...
# Load data
dat_resampled <- bind_rows(
  readRDS(file = paste0(dir_base,"/application/data/resampled_RR2/",asset,"_prices_resampled_starting2012.rds")),
  readRDS(file = paste0(dir_base,"/application/data/resampled_RR2/",asset,"_prices_HTS_resampled_starting2012.rds"))
)
...
dat_resampled_plot_nonHTS <- dat_resampled %>%
...
# Now for HTS seperately
dat_resampled_plot_HTS <- dat_resampled %>%
  dplyr::filter(sampling %in% c("HTS"),
                delta <= 0.0016, delta >= 0.0015) %>%
  mutate(time_last_tick = time_last_tick + seconds(34200),
         sampling=factor(sampling, levels=c('HTS'))) %>%
  rename(Time = time_last_tick)

dat_resampled_plot <- bind_rows(dat_resampled_plot_nonHTS, dat_resampled_plot_HTS)
...
ggplot() +
  ...
  scale_colour_manual(values = c("#619CFF", "#00BA38", "#F8766D", "purple")) +
...
ggsave("application/plots/plot_SamplingSchemes_RR2.pdf", width=8, height=7)
