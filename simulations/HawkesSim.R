...
simulate_TTSV_Hawkes <- function(days=2,
  ...
  # if (!is.function(mu_func)) stop("mu_func must be a function of time.")
  # if (!is.numeric(a_pos) || a_pos < 0 || !is.numeric(a_neg) || a_neg < 0)
  #   stop("a_pos and a_neg must be nonnegative.")
  # if (!is.numeric(b) || b <= 0) stop("b must be positive.")
  ...
  for (i_day in 1:days){
    ...
    while (t < T_max) {
      ...
      if (runif(1) <= lambda_candidate / M) {
        ...
        # OLD: varsigma <- approx(x=0:(TT-1), y=intensities_diffusion_day$varsigma_diff, xout=t)$y
